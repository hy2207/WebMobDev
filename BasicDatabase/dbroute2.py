...
def main():
    # create form
    cform = CreateTask(prefix='createform')
    dform = DeleteTask(prefix='deleteform')
    uform = UpdateTask(prefix='updateform')
    reset = ResetTask(prefix='resetform')

    # response
    if cform.validate_on_submit() and cform.validate():
        return createTask(cform)                           # this calls the createTask function (above)
    if dform.validate_on_submit() and dform.validate():
        return deleteTask(dform)
    if uform.validate_on_submit() and uform.validate():
